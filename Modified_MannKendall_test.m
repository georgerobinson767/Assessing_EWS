function [tau, p, H] = Modified_MannKendall_test(X, alpha, alpha_ac)
    ...
    % modification 13/12/24
    max_NumLags = length(X_rank_order) - 1;
    numLags = min(n-1, max_NumLags);
    % disp(numel(X_rank_order))
    % disp(numLags)

    % Check for NaNs: Ensure there are no NaN (Not a Number) values in your data that might be causing issues:
    if any(isnan(X_rank_order)) 
        error('X_rank_order contains NaN values.'); 
    end
    % understand why NaN values are being produced in the EWS from the data
    % too

    [acf, ~, acf_bounds] = autocorr(X_rank_order, "NumLags", numLags, "NumSTD", z_ac);
